...
class LinearProgrammingPlanner:
    ...
    def instantiate_lp(self):
        ...
        # TODO: compute the total expected cost based on the occupancy measure self.x and the cost function self.cost

        if self.cost_bound is not None:
            # add the constraint to bound the expected cost 
    ...
    def set_policy(self, h, s):
        """
        this function sets the value of the policy at state s in time step h (self.policy[h][s])
        """
        
        # TODO: set the policy for the at state s in time step h based on the occupancy of each action
        # if the state is not visite set it to an uniform action distribution
        policy_h_s = np.zeros(self.na)

        # occupancy = self.get_occupancy(h, s)
        policy_h_s[0] = 1

        self.policy[h][s] = policy_h_s
